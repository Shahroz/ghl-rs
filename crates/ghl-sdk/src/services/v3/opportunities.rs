...
//! `opportunities` — typed methods for all 12 API v3 operations
//! in this module.
//!
//! Access via [`Ghl::v3`](crate::Ghl::v3)`().opportunities()`. These endpoints send `Version: v3`.
//!
//! Request and response types come from [`ghl_models::v3::opportunities`](https://docs.rs/ghl-models/latest/ghl_models/v3/opportunities/); every endpoint is also documented in the
...
use ghl_models::v3::opportunities as models;

/// Typed access to the `opportunities` API v3 surface (12 operations). Obtained via
/// [`Ghl::v3`](crate::Ghl::v3)`().opportunities()`.
#[derive(Debug, Clone)]
...
impl SearchOpportunityParams {
    ...
    fn to_query(&self) -> Vec<(String, String)> {
        ...
        if let Some(v) = &self.pipeline_id {
            q.push(("pipelineId".into(), v.to_string()));
        }
        if let Some(v) = &self.pipeline_stage_id {
            q.push(("pipelineStageId".into(), v.to_string()));
        }
...
impl OpportunitiesService {
    ...
    pub async fn update_opportunity(
        &self,
        id: &str,
        body: &models::UpdateOpportunityDtoV3,
    ) -> Result<models::GetPostOpportunitySuccessfulResponseDto> {
        let path = format!("/opportunities/{}", crate::services::encode(id));
        let query = Vec::new();
        self.client
