...
class ServiceGen:
    ...
    def render_method(self, op):
        ...
        version = f'Some("{op["version"]}")' if op["version"] else "None"
    ...
    def render(self):
        ...
        header = [
            ...
            f"//! `{mod_name}` — typed methods for all {len(self.ops)} API v2 operations",
            "//! in this module.",
            "//!",
            f"//! Access via [`Ghl::{self.rust_mod}`](crate::Ghl::{self.rust_mod})"
            if self.module not in HANDWRITTEN
            else f"//! These methods live on the same"
                 f" [`{self.svc_name()}`](crate::{self.rust_mod}::{self.svc_name()}) you get from"
                 f" [`Ghl::{self.rust_mod}`](crate::Ghl::{self.rust_mod}), alongside the"
                 f" hand-written helpers.",
            "//!",
            f"//! Request and response types come from [`ghl_models::v2::{self.rust_mod}`]"
            f"(https://docs.rs/ghl-models/latest/ghl_models/v2/{self.rust_mod}/);"
            ...
            "",
            ("use crate::client::Ghl;" if self.module not in HANDWRITTEN else ""),
            "use crate::error::Result;",
            f"use ghl_models::v2::{self.rust_mod} as models;",
            "",
        ]
        ...
        if self.module not in HANDWRITTEN:
            body += doc(
                f"Typed access to the `{mod_name}` API "
                f"({len(self.ops)} operations). Obtained via "
                f"[`Ghl::{self.rust_mod}`](crate::Ghl::{self.rust_mod}).",
                indent="",
            )
...
def main():
    ...
    for f in sorted((docs_root / "apps").glob("*.json")):
        ...
        if not g.ops:
            continue
        (out_dir / f"{g.rust_mod}.rs").write_text(g.render())
        gens.append(g)
    ...
    for g in sorted(gens, key=lambda x: x.module):
        lines.append(f'/// `{g.module}` — {len(g.ops)} operations.')
    ...
    total = sum(len(g.ops) for g in gens)
    print(f"generated {total} service methods across {len(gens)} modules")
    for g in sorted(gens, key=lambda x: -len(x.ops))[:8]:
        print(f"  {g.module}: {len(g.ops)}")
    return gens
